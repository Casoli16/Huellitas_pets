...
require_once ('../../helpers/emailConnexion2.php');
...
class RecuperacionHandler
{
    ...
    public function envioCorreo()
    {
        ...
        $link = 'http://localhost/Huellitas_pets/system_huellitas/views/admin/recuperacion_contra_3.html?c=' . urlencode($this->hash) . '&id=' . urlencode($this->idUsuario) . '&n=' . urlencode($this->nivel);
...
{   
    ...
    {
        $sql = 'SELECT id_admin, nombre_admin FROM administradores
        WHERE correo_admin = ?;';
        ...
        $this->nombre = $this->idUsuario['nombre_admin'];
        $this->idUsuario = $this->idUsuario['id_admin'];
    ...
    public function updateRow1()
    {
        ...
        $sql = 'UPDATE administradores SET recovery_code = ? WHERE id_admin = ?;';
        $params = array(
            $this->hash,
            $this->idUsuario
        );
    ...
    {
        $sql = 'UPDATE administradores SET clave_admin = ?, recovery_code = ? WHERE id_admin = ?;';
        $params = array(
            $this->contrasena,
            $this->hash,
            $this->idUsuario
